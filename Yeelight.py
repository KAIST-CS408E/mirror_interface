...
def handle_user_input(command_line):
	'''
	...
	if argv[0] == "q" or argv[0] == "quit":
		...
		if len(argv) != 3:
			...
			idx = int(float(argv[1]))
			print "idx", idx
			color = int(float(argv[2])) * 65536 + int(float(argv[3])) * 256 + int(float(argv[4]))
			print "color", color
			set_color(idx, color)
...
@app.route('/ChangeLight', methods = ['GET', 'POST', 'DELETE'])

def ChangeLight():
	if request.method == 'POST':
		print(request.args.get("brightness"))
		print(request.args.get("toggle"))
		handle_user_input(request.args.get("brightness"))
		handle_user_input(request.args.get("color"))
		# handle_user_input("c 1 0 128 0")

		if request.args.get("toggle") == 'true':
			handle_user_input("t 1")
			handle_user_input("t 2")
...
if __name__ == '__main__':
	...
	# user interaction loop
	handle_user_input("l")
	handle_user_input("c 1 0 255 0")
	# handle_user_input("b 1 98")
